...
bool bounds_intersect_bounds(const bounds& p_bounds,
  ...
  vector3 aExtents = p_bounds.query_max() - aCenter;
  vector3 bExtents = within_bounds.query_max() - bCenter;
...
bool plane_intersect_plane(const plane& p1, const plane& p2, ray* out_ray) {
  vector3 p1_normal = p1;
  vector3 p2_normal = p2;
  ...
  if (p1_normal.parallel(p2_normal)) {
    ...
    if (p1 == p2) {
      if (out_ray) {
        out_ray->ray_start = vector3(p1.x, p1.y, p1.z) * (-p1.w);
        out_ray->ray_stop = out_ray->ray_start;
      }
  ...
  if (ray_intersect_plane(p2, temp_ray, &info)) {
    ...
    if (out_ray) {
      out_ray->ray_start = info.collision_point - direction * 500.0f;
      out_ray->ray_stop = out_ray->ray_start + direction * 1000.0f;
    }
...
bool point_in_triangle(const vector3& p1, const vector3& p2, const vector3& p3,
  ...
  return point_in_triangle(p1, p2, p3, plane, p_point);
}
...
bool ray_intersect_triangle(const vector3& p1, const vector3& p2,
                            ...
                            collision* hit_info) {
  //
  // Calculate a normal that 'faces' the ray.
  //

  vector3 normal = calculate_normal((p1), (p2), (p3));
  plane plane = calculate_plane(normal, (p1));

  if (normal.dot((p_ray.ray_stop - p_ray.ray_start)) < 0.0) {
    plane *= -1.0;
  }
...
bool ray_intersect_triangle(const vector3& p1, const vector3& p2,
  ...
  if (p_ray.has_zero_length()) {
    if (point_in_triangle(p1, p2, p3, p, p_ray.ray_start)) {
      if (hit_info) {
        vector3 normal = calculate_normal((p1), (p2), (p3));
        ...
        hit_info->collision_param = 0.0;
        hit_info->surface_normal = normal;
        hit_info->collision_point = p_ray.ray_start;
      }
  ...
  if (possibleHit.collision_param < 0 || possibleHit.collision_param > 1) {
    return false;
  }

  vector3 rayVec = p_ray.ray_stop - p_ray.ray_start;
  vector3 hitPointOnPlane = possibleHit.collision_point;
...
bool vnRayIntersectQuad(const vector3& p1, const vector3& p2, const vector3& p3,
...
bool ray_intersect_sphere(const vector3& pSphereCenter, float32 sphereRadius,
  ...
  //
  // If a point is on the surface of a sphere, its distance from the center is
  // equal to the radius. In other words, for a sphere centered at the origin:
  //
  // (x)^2 + (y)^2 + (z)^2 = r^2
  //
  // Thus for a sphere centered about some arbitrary point <h,k,l>:
  //
  // (x-h)^2 + (y-k)^2 + (z-l)^2 = r^2
  //
  // To find the intersection point, we simply plug in the equation of our ray,
  //
  // ray = point + t * vector
  //
  // and solve for t. This will define the t along our ray where an intersection
  // occurs with the sphere, if one actually does occur. If 0 <= t <= 1, then
  // the hit occurred at a valid point along our ray.
  //
  // Thus:
  //
  // r^2 = ((startx + t*(endx-startx))-h)^2 + ((starty + t*(endy-starty))-k)^2 +
  //		 ((startz + t*(endz-startz))-l)^2
  //
  // r^2 = (startx + t*vecx - h)^2 + (starty + t*vecy - k)^2 + (startz + t*vecz
  // - l)^2
  //
  // let xh = startx - h
  // let yk = starty - k
  ...
  // let rs = r^2
  //
  // rs = (t*vecx + xh)^2 + (t*vecy + yk)^2 + (t*vecz + zl)^2
  // this solves to quadratic equation au^2 + bu + c = 0
  //

  vector3 ray_vec = p_ray.ray_stop - p_ray.ray_start;
  vector3 ray_origin_vec = p_ray.ray_start - (pSphereCenter);

  float32 a = ray_vec.dot(ray_vec);
  float32 b = 2 * (ray_origin_vec.dot(ray_vec));
  ...
  //
  // If d < 0, no collision was detected
  //
  ...
  if (hit_info) {
    vector3 hitPos = p_ray.ray_start + (p_ray.ray_stop - p_ray.ray_start) * t;

    hit_info->collision_param = t;
    hit_info->collision_point = hitPos;
    hit_info->surface_normal = (hitPos - pSphereCenter).normalize();
  }
...
bool ray_intersect_bounds_worker(const bounds& p_bounds, const vector3& pStart,
  ...
  if (hit_info) {
    hit_info->collision_param = fnear;
    hit_info->collision_point = rayStart + rayVector * fnear;
    hit_info->surface_normal = hitNormal;
  }
...
bool ray_intersect_bounds(const bounds& p_bounds, const ray& p_ray,
  ...
  if (p_ray.has_zero_length()) {
    if (point_in_bounds(p_bounds, p_ray.ray_start)) {
      if (hit_info) {
        hit_info->collision_param = 0.0f;
        hit_info->collision_point = p_ray.ray_start;
        hit_info->surface_normal = vector3(0, 0, 0);
      }
  ...
  if (point_in_bounds(p_bounds, p_ray.ray_start) &&
      point_in_bounds(p_bounds, p_ray.ray_stop)) {
    if (hit_info) {
      hit_info->collision_param = 0.0f;
      hit_info->collision_point = p_ray.ray_start;
      hit_info->surface_normal = vector3(0, 0, 0);
  ...
  if (point_in_bounds(p_bounds, p_ray.ray_start)) {
    if (ray_intersect_bounds_worker(p_bounds, p_ray.ray_stop, p_ray.ray_start,
                                    hit_info)) {
      //
      ...
      if (hit_info) {
        hit_info->collision_param = 1.0f - hit_info->collision_param;
        hit_info->surface_normal *= -1.0f;
      }
  ...
  return ray_intersect_bounds_worker(p_bounds, p_ray.ray_start, p_ray.ray_stop,
                                     hit_info);
}
...
bool ray_intersect_plane(const plane& p, const ray& p_ray,
  ...
  if (point_in_plane(p, p_ray.ray_start)) {
    if (hit_info) {
      hit_info->collision_param = 0.0f;
      hit_info->collision_point = p_ray.ray_start;
      hit_info->surface_normal = vector3(p.x, p.y, p.z);
    }
  ...
  // point, then
  //      it has no chance of intersecting our plane.
  //
  ...
  vector3 ray_vec = p_ray.ray_stop - p_ray.ray_start;
  float32 ns = (p.x * p_ray.ray_start.x + p.y * p_ray.ray_start.y +
                p.z * p_ray.ray_start.z + p.w) *
               -1.0;
  float32 ts = (p.x * ray_vec.x + p.y * ray_vec.y + p.z * ray_vec.z);
  ...
  // because this would indicate that our
  //       ray began on the plane -- a case which we've already tested.
  //
  ...
  if (hit_info) {
    hit_info->collision_param = t;
    hit_info->collision_point = p_ray.ray_start + ray_vec * t;
    hit_info->surface_normal = vector3(p.x, p.y, p.z);
  }
